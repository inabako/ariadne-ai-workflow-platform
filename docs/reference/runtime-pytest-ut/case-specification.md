...
| --- | ---: |
| pytest files | 34 |
| pytest test functions | 562 |
| pytest collected cases | 575 |
...
ケース本文はsource fileごとに cases/ 配下へ分離しています。同期チェックでは、この索引ファイルと cases/*.md を結合してpytest収集結果と照合します。
...
- テスト関数を追加、削除、renameした場合は、この575ケース仕様書も更新します。
