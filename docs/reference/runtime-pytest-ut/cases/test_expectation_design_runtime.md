...
- Confirm: Expectation weight が正規化され、Critical expectation 違反が候補推薦を止めることを確認します。
- Input:
...
- Expected: candidate score、critical violation count、recommendable flag が expectation evaluator contract と一致します。
...
- Confirm: Design expectation violation detection が critical、major、minor、ambiguous、unverified、positive-surprise risk を分類することを確認します。
- Input:
  - pytest node: 上記 node id
  ...
  - parameter: names=なし case=なし
  - inline input: `expectations`
- Expected: violation severity と summary count が設計risk categoryを網羅します。
...
- Confirm: `aiwfctl design expectation` が sample artifact の初期化、候補比較、Human Gate decision 記録を実行できることを確認します。
- Input:
  - pytest node: 上記 node id
...
- Expected: JSON artifact、comparison report section、artifact index entry、approved Human Gate output が生成されます。
...
- Confirm: Expectation-Driven Design の source artifact が JSON のみで生成されることを確認します。
- Input:
  - pytest node: 上記 node id
...
- Expected: 必要な JSON artifact が存在し、YAML variant は存在せず、comparison が成功します。
...
- Confirm: 残りの expectation design command が scaffold、feasibility、extraction、review、refinement、contract、verification、feedback、dispatch artifact を生成することを確認します。
- Input:
  - pytest node: 上記 node id
...
- Expected: workflow command が期待される status code で完了し、schema-backed artifact と event log entry が生成されます。
...
- Confirm: Agent extraction request と Review Council sync が design comparison と Human Gate 判断材料へ反映されることを確認します。
- Input:
  - pytest node: 上記 node id
...
- Expected: agent output が取り込まれ、dispatch が期待 reviewer を選択し、council feedback が comparison report を更新し、未解決blocking issueはgateをpendingにします。
...
- Confirm: Expectation-Driven Design schema constant が実在する JSON Schema file を指すことを確認します。
- Input:
  - pytest node: 上記 node id
...
- Expected: 登録済み schema file が読み込め、draft 2020-12 の object schema を宣言しています。
