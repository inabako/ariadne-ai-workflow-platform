...
??????? `runtime/tests/test_workflow_doctor.py` ?pytest node id??UT?????
...
- 確認内容: pytest case `run git allows returncode one and filters blank lines` ?????runtime?????????????error boundary???????
- 入力値:
...
- 確認内容: pytest case `run git raises for unexpected returncode` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `tracked policy violations allows work readme but blocks rag files` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `missing required files reports core runtime assets` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `human gate registry flags schema responsibility boundary` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `human gate registry findings accepts missing or valid registry` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `close archive findings reports partial archive` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `close archive findings accepts missing root and complete archive` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `vscode utf8 first findings accepts complete settings` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `vscode utf8 first findings reports missing contract parts` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `vscode utf8 first findings reports missing or invalid settings` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `vscode utf8 first findings reports wrong terminal shapes and editorconfig snippets` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `vscode utf8 first findings ignores non powershell profiles` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `duckdb read model findings reports missing read model when sources exist` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `duckdb read model findings accepts missing sources or existing db` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `workflow doctor fail on warning turns warning into fail` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `workflow doctor run reports all warning types` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `workflow doctor run passes without warnings` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `workflow doctor main prints pass json` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `workflow doctor main returns one on fail on warning` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `workflow doctor ut spec sync findings and skip` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `defensive specimen workflow doctor reports missing ut spec inputs` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `defensive specimen workflow doctor reports stale and bad position only` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `defensive specimen workflow doctor reports stale without bad position` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
...
- 確認内容: pytest case `defensive specimen workflow doctor accepts clean ut spec sync` ?????runtime?????????????error boundary???????
- 入力値:
  - pytest node: 上記コードブロックのnode id
