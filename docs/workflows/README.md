...
| `/realtime-iac` | リアルタイムシステム向けIaC、開発・CI/CD・監視platform基盤、またはDB基盤を設計、生成、レビュー、検証、文書化する | 完成版要件定義書、共有通信/port/network成果物、platform component inventory、database component inventory | `work/<receipt-id>/`, IaC artifacts |
...
| Docker Compose、systemd、firewall、reverse proxy、監視、GitLab / Jenkins / Grafana / Zabbix基盤、またはPostgreSQL / MySQL基盤を整備したい | [Realtime IaC](realtime-iac.md) |
