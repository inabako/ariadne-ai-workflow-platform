...
class SQLiteJobStore:
    ...
    def save(self, job: Job) -> None:
        ...
        with self._connect() as connection:
            connection.execute(
                """
                insert into jobs(job_id, goal, workflow_name, state, payload)
                values (?, ?, ?, ?, ?)
                ...
                """,
                (job.job_id, job.goal, job.workflow_name, job.state.value, payload),
            )

    def load(self, job_id: str) -> Job:
        with self._connect() as connection:
            row = connection.execute(
                "select goal, workflow_name, state, payload from jobs where job_id = ?",
                (job_id,),
            ).fetchone()
        if row is None:
            raise KeyError(job_id)
        goal, workflow_name, state, payload_text = row
