...
from local_model_mcp.adapters.inbound.fastmcp import FastMCPServerAdapter
...
def test_fastmcp_adapter_delegates_to_application_use_case(tmp_path: Path) -> None:
    server = make_server(tmp_path)
    adapter = FastMCPServerAdapter(server.container)

    response = adapter.call_tool("invoke_local_model", {"prompt": "adapter boundary", "max_tokens": 20})

    assert adapter.mcp is None
    assert response["status"] == "ok"
    assert response["model_id"] == "mock-local-model"
    assert "adapter boundary" in response["text"]


def test_fastmcp_dependency_is_limited_to_inbound_adapter() -> None:
    template_root = Path(__file__).resolve().parents[1]
    source_root = template_root / "src" / "local_model_mcp"
    offenders: list[str] = []

    for path in source_root.rglob("*.py"):
        text = path.read_text(encoding="utf-8")
        if "fastmcp" not in text.lower():
            continue
        relative = path.relative_to(source_root).as_posix()
        if not relative.startswith("adapters/inbound/fastmcp/"):
