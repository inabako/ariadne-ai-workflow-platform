...
`ingestion_optimizer.py` は、RAG吸収前のchunk候補を `ACCEPT / REWRITE / HUMAN_CHECK / REJECT` に分類し、`db/rag/evidence/ingestion` にEvidenceを保存します。通常は `ACCEPT` 済みの `rag/optimized-chunks/*.json` だけをindex / embedding対象にしてください。
